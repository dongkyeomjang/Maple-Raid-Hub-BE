...
public class DelayedNotificationScheduler {
    ...
    private static final String PARTY_PENDING_KEY = "notification:pending:party";
    ...
    @Async("notificationExecutor")
    @EventListener
    public void handlePartyChatMessageReceived(PartyChatMessageReceivedEvent event) {
        try {
            String member = event.recipientId().getValue().toString() + ":" + event.partyRoomId();
            double score = Instant.now().plusSeconds(DELAY_SECONDS).toEpochMilli();

            redisTemplate.opsForZSet().add(PARTY_PENDING_KEY, member, score);

            String previewKey = "notification:preview:party:" + member;
            String previewValue = event.senderNickname() + "|" + event.messagePreview();
            redisTemplate.opsForValue().set(previewKey, previewValue);
        } catch (Exception e) {
            log.error("[알림] 파티 채팅 알림 등록 실패: {}", e.getMessage(), e);
        }
    }

    public void cancelPendingPartyNotification(UserId userId, String partyRoomId) {
        try {
            String member = userId.getValue().toString() + ":" + partyRoomId;
            redisTemplate.opsForZSet().remove(PARTY_PENDING_KEY, member);
            redisTemplate.delete("notification:preview:party:" + member);
        } catch (Exception e) {
            log.error("[알림] 파티 채팅 알림 취소 실패: {}", e.getMessage(), e);
        }
    }

    @Scheduled(fixedRate = 30000)
    public void processExpiredPartyNotifications() {
        try {
            double now = Instant.now().toEpochMilli();
            ZSetOperations<String, String> zSetOps = redisTemplate.opsForZSet();

            Set<String> expired = zSetOps.rangeByScore(PARTY_PENDING_KEY, 0, now);
            if (expired == null || expired.isEmpty()) {
                return;
            }
            ...
            for (String member : expired) {
                try {
                    processSinglePartyNotification(member);
                } catch (Exception e) {
                    log.error("[알림] 파티 채팅 알림 처리 실패 member={}: {}", member, e.getMessage(), e);
                } finally {
                    zSetOps.remove(PARTY_PENDING_KEY, member);
                    redisTemplate.delete("notification:preview:party:" + member);
                }
            }
        } catch (Exception e) {
            log.error("[알림] 파티 채팅 스케줄러 실행 실패: {}", e.getMessage(), e);
        }
    }

    private void processSinglePartyNotification(String member) {
        String[] parts = member.split(":", 2);
        if (parts.length != 2) return;

        UserId userId = UserId.of(parts[0]);
        String partyRoomId = parts[1];

        PartyRoom partyRoom = partyRoomRepository.findById(PartyRoomId.of(partyRoomId))
                .orElse(null);
        if (partyRoom == null || partyRoom.getUnreadCount(userId) == 0) {
            return;
        }
        ...
        if (!pref.isNotifyPartyChatReceived()) {
            return;
        }
