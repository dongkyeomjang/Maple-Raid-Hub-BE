package com.mapleraid.character.domain.resolver;
...
public class WorldGroupResolver {
     ...
     * 월드명으로 월드 그룹 조회
     */
    public EWorldGroup resolve(String worldName) {
        if (CHALLENGER_WORLDS.contains(worldName)) {
            return EWorldGroup.CHALLENGER;
        }
        if (EOS_HELIOS_WORLDS.contains(worldName)) {
            return EWorldGroup.EOS_HELIOS;
        }
        if (NORMAL_WORLDS.contains(worldName)) {
            return EWorldGroup.NORMAL;
        }
     ...
     * 두 월드 그룹이 매칭 가능한지 확인
     */
    public boolean canMatch(EWorldGroup group1, EWorldGroup group2) {
     ...
     * 월드 그룹에 포함된 모든 월드 목록
     */
    public Set<String> getWorldsInGroup(EWorldGroup group) {
        return switch (group) {
